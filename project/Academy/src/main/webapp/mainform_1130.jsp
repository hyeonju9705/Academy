...
System.out.print("ok는??? "+request.getAttribute("ok"));
out.print("ok는??? "+request.getAttribute("ok"));
if(request.getHeader("referer").equals("http://localhost:9000/Academy/login.jsp")){

System.out.print("참참참");
if ((int) request.getAttribute("ok") == 1) {
	session.setAttribute("user", request.getAttribute("user"));
	System.out.print("1111");
	
	//session.setAttribute("userId", request.getParameter("userId")); //왜있는거지
	session.setAttribute("userId", "<c:out value='${user.userId}'/>"); //userId라는 이름으로 session생성
	%>
     <%--  // alert('환영합니다. 아카데미 홈페이지에 방문해주셔서 감사합니다. user: '${user}); --%>
 <script>alert('환영합니다. ')</script>
<%} else if ((int) request.getAttribute("ok") == 2) {%>
<!-- alert('비밀번호가 틀렸습니다. 확인 후 로그인 바랍니다.'); -->
    <%-- location.href='<%=request.getContextPath()%>/login.jsp';  --%>
<%} else {%>
     <%--  alert('아이디가 틀렸습니다. 확인 후 로그인 바랍니다.');
				...
				<ol>
					<c:forEach var="Aitem" items="${Aadao }">
						<li
							...
							<a href='AdetailViewProc.do?boardNo=${boardNo }'>[전체] ${Aitem.title} </a> <!-- ex -->
						</li>
					</c:forEach>
